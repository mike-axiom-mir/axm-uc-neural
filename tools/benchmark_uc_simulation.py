"""Compare equal-budget curricula across three predetermined brain seeds."""
import argparse
import json
...
from run_uc_simulation_lab import ROOT,load_dependencies,fresh_session,providers,atomic_write,source_identity
...
def run(episodes=768,full_dir=None):
    ...
    for seed in (17,41,73):
        for policy in SimulationSession.POLICIES:
            ...
            assert after==retained and restored.to_snapshot()==snapshot
            ...
            row = {'brain_seed':seed,'policy':policy,'training_transitions':session.transitions,
                   'before':before,'after':after,'retained':retained,'restore_exact':True,
            ...
            print(json.dumps({'seed':seed,'policy':policy,'before':before['mean_family_mse'],
                              'after':after['mean_family_mse'],'counts':session.counts}),flush=True)
    means = {policy:sum(row['after']['mean_family_mse'] for row in rows if row['policy']==policy)/3
             for policy in SimulationSession.POLICIES}
    return {'schema':'axm.uc-simulation-comparison/v1','rows':rows,'mean_held_out_mse':means,
            ...
            'limits':['All families expose the same UC rectangle-fitting rule with different input distributions.',
                      'error_guided is a moving-error heuristic, not a learned neural meta-selector.',
                      'No external-task, creative-quality, general-intelligence or WALDO-learning claim.',
