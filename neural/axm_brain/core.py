...
class Experience:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "source": self.source,
            "tag": self.tag,
            "directions": list(normalize_directions(self.directions)),
        }
    ...
    def from_dict(cls, data: dict) -> "Experience":
        return cls(
            ...
            else [float(x) for x in data["target"]],
            reward=None
            if data.get("reward") is None
            else float(data["reward"]),
            source=str(data.get("source", "host")),
            tag=str(data.get("tag", "")),
            directions=normalize_directions(data.get("directions", ())),
        )
...
class AXMBrain:
    ...
    def __init__(self, config: BrainConfig):
        ...
        self.w_in = self._random_matrix(
            config.hidden_size, config.input_size, scale_in
        )
        self.w_rec = self._random_matrix(
            config.hidden_size, config.hidden_size, scale_h
        )
        self.w_out = self._random_matrix(
            config.output_size, config.hidden_size, scale_h
        )
        ...
        self.direction_experience_counts = {
            direction: 0 for direction in ROOT_DIRECTIONS
        }
        self.untagged_experience_count = 0

    def _random_matrix(
        self, rows: int, cols: int, scale: float
    ) -> list[list[float]]:
        return [
    ...
    def _validate_vector(
        values: Iterable[float], size: int, name: str
    ) -> list[float]:
    ...
    def predict(
        self, observation: Iterable[float], *, update_state: bool = True
    ) -> list[float]:
        obs = self._validate_vector(
            observation, self.config.input_size, "observation"
        )
        ...
        for i in range(self.config.hidden_size):
            ...
            total += sum(
                w * h for w, h in zip(self.w_rec[i], previous_hidden)
            )
            hidden.append(tanh(total))
        output = []
        for o in range(self.config.output_size):
            total = self.b_out[o] + sum(
                w * h for w, h in zip(self.w_out[o], hidden)
            )
    ...
    def experience(
        self, event: Experience, *, remember: bool = True
    ) -> list[float]:
        if self.mode != "wake":
            raise RuntimeError(
                "direct experience is accepted only in wake mode"
            )
        normalized = Experience.from_dict(event.to_dict())
        self.host_experience_count += 1
        if normalized.directions:
            for direction in normalized.directions:
                self.direction_experience_counts[direction] += 1
        else:
            self.untagged_experience_count += 1
        output = self.predict(normalized.observation, update_state=True)
        if normalized.target is not None:
            self._supervised_update(
                normalized.observation, normalized.target, scale=1.0
            )
        if normalized.reward is not None:
            self._reward_update(float(normalized.reward))
        if remember and self.config.replay_capacity > 0:
            self.replay.append(normalized)
            if len(self.replay) > self.config.replay_capacity:
                del self.replay[
                    : len(self.replay) - self.config.replay_capacity
                ]
        return output
    ...
    def _update_eligibility(
        ...
        for i in range(self.config.hidden_size):
            for j in range(self.config.input_size):
                self.trace_in[i][j] = (
                    d * self.trace_in[i][j] + hidden[i] * obs[j]
                )
            for j in range(self.config.hidden_size):
                self.trace_rec[i][j] = (
                    d * self.trace_rec[i][j]
                    + hidden[i] * previous_hidden[j]
                )
        for o in range(self.config.output_size):
            for i in range(self.config.hidden_size):
                self.trace_out[o][i] = (
                    d * self.trace_out[o][i]
                    + self.last_output[o] * hidden[i]
                )
    ...
    def _supervised_update(
        ...
        *,
        ...
        target_vec = self._validate_vector(
            target, self.config.output_size, "target"
        )
        lr = self.config.learning_rate * scale
        out_errors = [
            target_vec[o] - self.last_output[o]
            for o in range(self.config.output_size)
        ]
        hidden_errors = [0.0] * self.config.hidden_size
        for i in range(self.config.hidden_size):
            hidden_errors[i] = sum(
                out_errors[o] * self.w_out[o][i]
                for o in range(self.config.output_size)
            )
        ...
        for o in range(self.config.output_size):
            ...
            for i in range(self.config.hidden_size):
                self.w_out[o][i] = _clip(
                    self.w_out[o][i]
                    + lr * grad * self.hidden[i],
                    self.config.weight_limit,
                )
            self.b_out[o] = _clip(
                self.b_out[o] + lr * grad,
                self.config.weight_limit,
            )
        ...
        for i in range(self.config.hidden_size):
            ...
            for j in range(self.config.input_size):
                self.w_in[i][j] = _clip(
                    self.w_in[i][j]
                    + lr * grad * observation[j],
                    self.config.weight_limit,
                )
            self.b_hidden[i] = _clip(
                self.b_hidden[i] + lr * grad,
                self.config.weight_limit,
            )
        ...
        for _ in range(self.config.sleep_replay_passes):
            for event in list(self.replay):
                ...
                if event.reward is not None:
                    self._reward_update(
                        float(event.reward)
                        * self.config.sleep_learning_scale
                    )
                replayed += 1
        pruned = self._prune_small_weights()
        self.hidden = before_hidden
        self.sleep_count += 1
        self.sleep_replay_event_count += replayed
        self.total_pruned_weights += pruned
        return {
            "replayed": replayed,
            "pruned": pruned,
            "sleep_count": self.sleep_count,
    ...
    def to_snapshot(self) -> dict:
        body = {
            ...
            "state": {
                ...
                "steps": self.steps,
                "sleep_count": self.sleep_count,
                "host_experience_count": self.host_experience_count,
                "supervised_update_count": self.supervised_update_count,
                "reward_update_count": self.reward_update_count,
                "sleep_replay_event_count": self.sleep_replay_event_count,
                "total_pruned_weights": self.total_pruned_weights,
                "direction_experience_counts": self.direction_experience_counts,
                "untagged_experience_count": self.untagged_experience_count,
            },
        }
    ...
    def from_snapshot(cls, snapshot: dict) -> "AXMBrain":
        ...
        if body.get("schema") != cls.SCHEMA:
            raise ValueError(
                f"unsupported brain schema: {body.get('schema')}"
            )
        ...
        for name in (
            ...
            setattr(
                brain,
                name,
                [[float(v) for v in row] for row in state[name]],
            )
        ...
        brain.replay = [
            Experience.from_dict(x) for x in state["replay"]
        ]
